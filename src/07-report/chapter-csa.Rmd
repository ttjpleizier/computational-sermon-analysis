...
```{r kwic, echo=FALSE, message=FALSE, warning=FALSE}
example_kwic
```


In sum, frequency tables entail more complex techniques than simple word counts. Further, the examples of 'st' and 'God's' also show that the term 'word' is not unambiguous. However, frequency tables are the starting point for other computations, such as keyness and lexical diversity (section 5).
...
[^1]: Spurgeon's sermons are available at https://ccel.org. The database started in 1997 with a CD-ROM containing classical Christian literature, which included 63 volumes of sermons by Spurgeon.  
[^2]: In recent years more of his sermons have been rediscovered, published in a series *The Lost Sermons* (7 volumes between 2017 and 2022). This collection consists of photographs of Spurgeon's handwritten sermons. 
[^3]: The *Parochial and Plain Sermons* from Newman were scraped from https://newmanreader.org/. The full digital collection of Newman's works can be found at NINS, https://digitalcollections.newmanstudies.org/. 
[^4]: Web scraping is a method used in  data science and digital humanities to extract data from the internet. Cf. [@black2016]. Black argues that the Internet could be used as a low-cost and resource-light source for research data. The scripts used to scrape the sermons from Newman and Spurgeon are published at the Github repository that belongs to this article, see further below, section XXX. He also discusses the legal aspects of webscraping for academic purposes. 
...
[^6]: The streaming services https://kerkomroep.nl and https://kerkdienstgemist.nl  (accessed 28 November 2023) provide access to worship services of around 3,000 churches . See for these services further [@pleizier2023a].  The estimation of 3,000-5,000 sermons per week acknowledges that a considerable amount of Dutch churches offer two services a Sunday. 
